...
function merge(alphaFirst, alphaSecond) {
  ...
  for(let i=0; i < catPixels.length;i++){
      ...
      for(let j=0; j < catPixels[i].length; j++){
        ...
        pixelCat[2]=pixelCat[2]*alphaSecond; 
        let pixelDog=dogPixels[i][j];
        pixelDog[0]=pixelDog[0]*alphaFirst;
        pixelDog[1]=pixelDog[1]*alphaFirst;
        pixelDog[2]=pixelDog[2]*alphaFirst;
        let pixel =[];
        pixel[0]=pixelCat[0]+pixelDog[0];
        pixel[1]=pixelCat[1]+pixelDog[1];
        pixel[2]=pixelCat[2]+pixelDog[2];
        pixelsFinal.push(pixel);

      }
    pixels.push(pixelsFinal);
  }
