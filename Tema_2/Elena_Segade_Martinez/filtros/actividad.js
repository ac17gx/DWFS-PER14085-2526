...
function redConverter() {
    ...
    let rows = handler.shape[0];
    let columns = handler.shape[1];
...
function scaleDown() {
    ...
    handler.savePixels(newImage, outputPath, handler.shape[0] / 2, handler.shape[1] / 2);
}
...
function merge(alphaFirst, alphaSecond) {
    ...
    //Aqui tu codigo
    let rows = catHandler.shape[0];
    let columns = catHandler.shape[1];
