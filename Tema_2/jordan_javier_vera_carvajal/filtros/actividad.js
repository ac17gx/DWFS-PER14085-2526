...
function greyConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
      for (let j = 0; j < pixels[i].length; j++){
        let mediaPixel = (pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2]) / 3;
        pixels[i][j][0] = mediaPixel;
        pixels[i][j][1] = mediaPixel;
        pixels[i][j][2] = mediaPixel;
      }
  }
...
function scaleDown() {
  ...
  //Aqui tu codigo
  const filasImpares = pixels.filter((filas, posicionFila) => {
      return posicionFila % 2 !== 0; 
  });

  const pixelesFiltrados = filasImpares.map(filas => {
    return filas.filter((pixel, posicionColumna) => {
            return posicionColumna % 2 !== 0; 
    });
  });

  handler.savePixels(pixelesFiltrados, outputPath, handler.getShape()[0] / 2, handler.getShape()[1] / 2);
}
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i++) {
      for (let j = 0; j < catPixels[i].length; j++){
          ...
          catPixels[i][j][0] = (catPixels[i][j][0] * alphaFirst) + (dogPixels[i][j][0] * alphaSecond);
          catPixels[i][j][1] = (catPixels[i][j][1] * alphaFirst) + (dogPixels[i][j][1] * alphaSecond);
          catPixels[i][j][2] = (catPixels[i][j][2] * alphaFirst) + (dogPixels[i][j][2] * alphaSecond);
       
      }
  }

  dogHandler.savePixels(catPixels, outputPath);
}
