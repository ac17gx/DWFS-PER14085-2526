...
import com.api.calculator.repository.OperacionRepository;
...
public class OperacionService {
    ...
    public OperacionService(OperacionRepository repo, OperacionIdGenerator idGenerator) {
        this.repo = repo;
        this.idGenerator = idGenerator;
    }

    public Operacion crear(OperacionCreateRequest req) {
        ...
        Operacion op = new Operacion(id, req.tipo(), req.entradas(), params, resultado, creadoEn);
        return repo.save(op);
    }

    public Operacion obtener(String id) {
    ...
    private void validarReglas(TipoOperacion tipo, List<BigDecimal> entradas, Parametros parametros) {
        ...
        switch (tipo) {
            case SUMA, RESTA -> {
                if (entradas.size() < 2) {
                    throw new UnprocessableEntityException(tipo + " requiere mínimo 2 entradas.");
                }
            }
            case MULTIPLICACION, DIVISION -> {
                if (entradas.size() != 2) {
                    throw new UnprocessableEntityException(tipo + " requiere exactamente 2 entradas.");
                }
            ...
            case RAIZ -> {
                if (entradas.size() != 1) {
                    throw new UnprocessableEntityException("RAIZ requiere exactamente 1 entrada.");
                }
            ...
            case POTENCIA -> {
                if (entradas.size() != 1) {
                    throw new UnprocessableEntityException("POTENCIA requiere exactamente 1 entrada.");
                }
    ...
    private Map<String, Object> normalizarParametros(TipoOperacion tipo, Parametros parametros) {
        ...
        if (tipo == TipoOperacion.RAIZ) {
            map.put("grado", parametros != null ? parametros.grado() : null);
        } else if (tipo == TipoOperacion.POTENCIA) {
            map.put("exponente", parametros != null ? parametros.exponente() : null);
        }
        return map;
    }
